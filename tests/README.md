...
| `test_request_body.nu` | Structured JSON/form interpolation boundaries, null preservation, body-file injection safety, table replay/chains, recursively resolved static chain templates with opaque extracted values through run/exec, literal header names, preview/export fidelity, and credential masking |
