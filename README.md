...
Workspace state replacements write complete bytes to a unique dotted sibling in the destination directory, then publish with a native same-filesystem move. Sibling temps and replacements inherit the destination directory's access policy; custom per-file Windows DACLs and POSIX mode/extended ACL/owner/group settings are not retained, so users requiring custom access control must secure the workspace directory. New-file creates write directly without a temp: interruption can leave a partial new file, but no pre-existing bytes are at risk and readers report corruption actionably. A hard-killed replacement can leave a full dotted sibling temp; a later mutation of that exact destination removes matching temps older than one hour when possible. If removal fails, Nurl prints a path-only manual-remedy warning to stderr while the requested write still succeeds with its normal stdout. Nurl runs no cleanup daemon, and external writers do not participate in advisory coordination.
