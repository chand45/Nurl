...
Workspace configuration, credentials, variables, collections, environments, saved requests, and chains are committed through unique protected temp files on the same filesystem and native replacement. If a state file is corrupt or has the wrong shape, Nurl stops with a path-specific recovery error instead of exposing parser internals or continuing with partial state. Windows temp files use a protected current-user DACL. State writes replace the destination, so custom per-file Windows DACLs are not retained; users requiring custom access control must secure the workspace directory. POSIX replacements preserve mode bits, but not extended ACLs or ownership. A killed process can leave a protected temp in `.nurl-state`.
