...
| api history clear | In a throwaway `API_ROOT`, create deterministic old/future date directories and exercise `--before`, configured retention, and `--all --force`. | `--before` and retention remove only qualifying date directories; `--all --force` recreates an empty history directory. |
