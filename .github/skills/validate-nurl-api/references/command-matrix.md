...
| api history clear | In a throwaway `API_ROOT`, build a real index over deterministic removed/retained boundary fixtures; exercise `--before`, wide-margin non-default retention, parseable wrong-shaped indexes, blocked later directories, a blocked entry after an ordinary entry in the first qualifying directory, post-delete rebuild failure, duplicate/conflicting/stale/linked hints, and separate successful/partially failing `--all --force` cases. | Partial clears remove only qualifying date directories and rebuild canonical index state so index/list/URL search/body search/get/JSON/CSV agree exactly on retained IDs and order. Before mutation, clear validates and canonicalizes exact-schema index hints to unique contained regular `.nuon` paths; direct files win over logical aliases, and directory/non-NUON link targets are excluded. Every delete or post-delete rebuild failure remains nonzero without success output and reconciles those hints against contained surviving paths without opening locked entries. Compatible duplicates and injected fields are normalized; conflicting IDs/paths produce a composed reconciliation error without rewriting the index. Wrong-shaped indexes recover to a valid list, including empty. Successful `--all --force` recreates an entirely empty history directory with no index; a partial recursive failure recreates a canonical index for surviving hinted entries. |
