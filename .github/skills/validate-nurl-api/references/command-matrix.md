...
| api history clear | In a throwaway `API_ROOT`, build a real index over deterministic removed/retained boundary fixtures; exercise `--before`, wide-margin non-default retention, parseable wrong-shaped indexes, blocked later directories, a blocked entry after an ordinary entry in the first qualifying directory, and separate `--all --force` cases. | Partial clears remove only qualifying date directories and rebuild canonical index state so index/list/URL search/body search/get/JSON/CSV agree exactly on retained IDs and order. Every recursive-delete failure remains nonzero without success output and reconciles valid index hints against contained surviving paths without opening locked entries, including partial mutation of the first directory. Wrong-shaped indexes recover to a valid list, including empty. `--all --force` recreates an entirely empty history directory with no index. |
