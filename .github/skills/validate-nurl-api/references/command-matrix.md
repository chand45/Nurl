...
| api history search | Search the ten-entry burst by their shared URL fragment before and after index rebuild; exercise zero/negative/overlarge limits. | Results use the same reverse insertion order as list; limit behavior follows the shared history limit contract. |
...
| api history export | Export the ten-entry burst as JSON and CSV to stdout and files with limits `0`, `1`, exact count, and overlarge; delete/rebuild the index and repeat limit `1`. | Both formats use canonical newest-first index order and put the final persisted ID first before/after rebuild. JSON keeps the full entry schema; CSV remains `id,timestamp,method,url,status,time_ms`. Invalid formats and negative limits fail before output/index mutation. |
| api history rebuild-index | Build fixtures with fractional and legacy second-only RFC3339 UTC timestamps, tied timestamps, malformed/missing timestamps, and a ten-entry no-sleep burst; remove `index.nuon`, rebuild, then compare list/search/export. | Rebuild uses numeric timestamp order, keeps new-entry order, sorts malformed/missing timestamps deterministically last, and never rewrites entry files. Existing index order is retained for truly tied legacy timestamps when available; index-free ties use a deterministic fallback. |
| api history save | In an isolated `API_ROOT`, save a ten-entry no-sleep burst and synthetic request/response records containing inline auth, sensitive headers, credential-shaped unknown metadata, and userinfo/sensitive URL parameters; also exercise valid and invalid named refs. Inspect history bytes, index, list/search/show/get, JSON/CSV export, and replay. | New entries use unique monotonically ordered nine-digit fractional RFC3339 UTC timestamps derived with the date directory and ID time component from one captured clock instant. The shared live/direct boundary writes only canonical request/response fields, preserves response-body and safe URL semantics, stores non-replayable auth metadata and masked sensitive headers, and omits unknown metadata. Named refs and caller URLs are validated before mutation. Invalid schemas/refs/auth/URLs fail with empty stdout and unchanged state. Legacy unsafe URLs remain readable without migration but cannot resend. |
