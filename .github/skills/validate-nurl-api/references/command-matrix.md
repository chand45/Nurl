...
| Command | Invocation | Expected |
|---|---|---|
| api history list | In an isolated `API_ROOT`, save ten synthetic `request-1`...`request-10` entries without sleeping; run `api history list --limit 1`, full/zero/overlarge limits, then delete/rebuild the index and repeat. | The final persisted ID is first before and after rebuild; full output is exact reverse insertion order, `0` returns no rows, overlarge limits return all, and negative limits fail cleanly before index creation. Human timestamps remain `HH:MM:SS`. |
| api history show | Exercise exact IDs, unique prefix/middle/suffix fragments, and a fragment shared by multiple fixtures. | Exact wins, every unique partial prints the full request/response, and ambiguity exits nonzero with empty stdout, actionable non-ANSI stderr, and no selected record or mutation. |
| api history resend | `api history resend <id>` plus an ambiguous fragment against local endpoint/OAuth fixtures. | Re-executes unique entries; POST bodies are not double-encoded. Ambiguity fails before auth/network work. Named auth refs are re-resolved, inline auth requires an explicit `--auth` override, legacy no-auth entries remain unauthenticated, and `--dry-run` stays masked/side-effect-free. Transport failure has empty stdout and propagates nonzero. `-e` is **inert**. |
| api history search | Search the ten-entry burst by shared URL and response-body fragments before and after index rebuild; exercise zero/negative/overlarge limits. | URL and body results use the same exact reverse insertion order as list; limit behavior follows the shared history limit contract. |
| api history clear | In a throwaway `API_ROOT`, build a real index over deterministic removed/retained boundary fixtures; exercise `--before`, wide-margin non-default retention, parseable wrong-shaped indexes, blocked later directories, a blocked entry after an ordinary entry in the first qualifying directory, post-delete rebuild failure, duplicate/conflicting/stale/unsafe/linked hints, a dangling contained alias, and separate successful/partially failing `--all --force` cases. | Partial clears remove only qualifying date directories and rebuild canonical index state so index/list/URL search/body search/get/JSON/CSV agree exactly on retained IDs and order. Before mutation, clear captures exact index bytes and validates exact-schema hints to contained regular `.nuon` paths; invalid rows and dangling/external/non-file aliases are dropped individually, while direct files win over valid contained aliases. Every delete or post-delete rebuild failure remains nonzero without success output and reconciles surviving hints without opening locked entries. Compatible duplicates and injected fields are normalized; conflicts among surviving paths produce a composed reconciliation error. If reconciliation is impossible after the index changed or disappeared, its prior bytes are restored exactly. Wrong-shaped indexes recover to a valid list, including empty. Successful `--all --force` recreates an entirely empty history directory with no index; a partial recursive failure recreates a canonical index for surviving hinted entries. |
| api history export | Export the ten-entry burst as JSON and CSV to stdout and files with limits `0`, `1`, exact count, and overlarge; delete/rebuild the index and repeat the full export. | Both formats return the exact canonical newest-first ID sequence before/after rebuild. JSON keeps the full entry schema; CSV remains `id,timestamp,method,url,status,time_ms`, with header-only output for limit `0`. Invalid formats and negative limits fail before output/index mutation. |
| api history rebuild-index | Build fixtures with fractional and legacy second-only RFC3339 UTC timestamps, tied timestamps, malformed/missing timestamps, stale/duplicate index hints, parseable invalid top-level/index-row shapes, text/binary malformed entry content, an unreadable existing entry, and a ten-entry no-sleep burst; remove or corrupt `index.nuon`, rebuild, then compare list/search/export. | Rebuild uses numeric timestamp order, keeps new-entry order, drops stale/duplicate hints, normalizes structurally unusable indexes from entry files, skips only syntactically/structurally invalid entry content, and propagates traversal/read failures without replacing the prior index or printing success. Malformed/missing timestamps sort deterministically last and entry files are never rewritten. Existing valid index order is retained for truly tied legacy timestamps when available; index-free ties use a deterministic fallback. |
| api history save | In isolated `API_ROOT` workspaces with differing `default_environment` values, save a ten-entry no-sleep burst and synthetic request/response records containing inline auth, sensitive headers, credential-shaped unknown metadata, and userinfo/sensitive URL parameters; also exercise valid and invalid named refs. Inspect history bytes, index, list/search/show/get, JSON/CSV export, and replay. | New entries use unique monotonically ordered nine-digit fractional RFC3339 UTC timestamps derived with the date directory and ID time component from one captured clock instant. Entries retain the active root's environment without cross-root visibility. The shared live/direct boundary writes only canonical request/response fields, preserves response-body and safe URL semantics, stores non-replayable auth metadata and masked sensitive headers, and omits unknown metadata. Named refs and caller URLs are validated before mutation. Invalid schemas/refs/auth/URLs fail with empty stdout and unchanged state. Legacy unsafe URLs remain readable without migration but cannot resend. |
...
- **history:** `api history save` — the persistence boundary is covered directly by the
  synthetic-save security and deterministic burst regressions and transitively by live requests;
  `api history get` is covered by exact/unique/ambiguous resolver regressions and `history show`.
