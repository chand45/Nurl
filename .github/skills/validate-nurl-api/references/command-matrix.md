...
| api history clear | In a throwaway `API_ROOT`, build a real index over deterministic removed/retained boundary fixtures; exercise `--before`, wide-margin non-default retention, parseable wrong-shaped indexes, a platform-specific blocked second deletion, and separate `--all --force` cases. | Partial clears remove only qualifying date directories and rebuild canonical index state so index/list/URL search/body search/get/JSON/CSV agree exactly on retained IDs and order. If a later deletion fails, the command remains nonzero without success output while the index reflects earlier committed deletions. Wrong-shaped indexes recover to a valid list, including empty. `--all --force` recreates an entirely empty history directory with no index. |
...
| api history rebuild-index | Build fixtures with fractional and legacy second-only RFC3339 UTC timestamps, tied timestamps, malformed/missing timestamps, stale/duplicate index hints, parseable invalid top-level/index-row shapes, and a ten-entry no-sleep burst; remove or corrupt `index.nuon`, rebuild, then compare list/search/export. | Rebuild uses numeric timestamp order, keeps new-entry order, drops stale/duplicate hints, normalizes structurally unusable indexes from entry files, sorts malformed/missing timestamps deterministically last, and never rewrites entry files. Existing valid index order is retained for truly tied legacy timestamps when available; index-free ties use a deterministic fallback. |
